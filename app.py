...
if mode == "Data Collection Mode":
    if st.sidebar.button('Save Results to MySQL Database'):
        conn = sql_connection()
        if conn:
            cursor = conn.cursor()
            ...
            insert_output = """
                ...
                VALUES (%s,%s,%s,%s,%s)
            """
            cursor.execute(insert_output, (calculationInputId, sigma, S, price, price - purchase_price))
            conn.commit()
